 ...
 *
 * Lists a paginated set of trials from GET /api/filtered-studies.
 * Allows searching, sorting, and "Load More" pagination.
 */
...
const TrialsPage: React.FC = () => {
  // Hook calls /api/filtered-studies
  const { trials, loading, error, nextPageToken, fetchNextPage } = useTrials({
    format: "json",
    pageSize: 20,
    fields: "NCTId,BriefTitle,OverallStatus,HasResults,protocolSection.conditionsModule",
  });

  const [searchTerm, setSearchTerm] = useState("");
  const [sortField, setSortField] = useState<"briefTitle" | "overallStatus" | "condition">(
    "briefTitle"
  );
  ...
  useEffect(() => {
    console.log("[TrialsPage] Trials data:", trials);
  }, [trials]);
    ...
    >
      ...
      <div className="flex flex-col md:flex-row items-center md:justify-between mb-6 space-y-4 md:space-y-0">
        ...
        <div className="flex space-x-2">
          ...
          >
            Sort by Title{" "}
            {sortField === "briefTitle" && (sortOrder === "asc" ? "↑" : "↓")}
          </button>
          <button
            className="btn btn-ghost text-[#434B56] hover:text-[#6A9EFD] transition"
          ...
          >
            Sort by Status{" "}
            {sortField === "overallStatus" && (sortOrder === "asc" ? "↑" : "↓")}
          </button>
          <button
            className="btn btn-ghost text-[#434B56] hover:text-[#6A9EFD] transition"
