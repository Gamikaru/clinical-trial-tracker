 ...
 *
 * Demonstrates advanced parameters, calling /api/filtered-studies with
 * condition, sponsor, location, status, etc.
 */
...
const AdvancedSearchPage: React.FC = () => {
  ...
  // Connect to the local endpoint for advanced searching
  const { trials, loading, error, nextPageToken, fetchNextPage, resetResults } = useTrials({
    pageSize: 10,
    ...searchParams,
  });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex flex-wrap gap-4 mb-4">
          <div className="w-full sm:w-1/3">
            <label className="block mb-2 font-semibold">Latitude (optional)</label>
            <input
              type="number"
              step="any"
          ...
          <div className="w-full sm:w-1/3">
            <label className="block mb-2 font-semibold">Longitude (optional)</label>
            <input
              type="number"
              step="any"
              className="input input-bordered w-full"
          ...
          <div className="w-full sm:w-1/3">
            <label className="block mb-2 font-semibold">Radius (e.g. 50mi)</label>
            <input
              type="text"
              className="input input-bordered w-full"
        ...
        <div className="flex flex-wrap gap-4 mb-4">
          {["NOT_YET_RECRUITING", "RECRUITING", "COMPLETED", "SUSPENDED", "TERMINATED"].map(
            (stat) => (
              <label key={stat} className="inline-flex items-center cursor-pointer">
                <input
                  type="checkbox"
      ...
      <div className="my-4 text-sm" style={{ color: "#666" }}>
        ...
        {searchParams.condition && <span>Condition = {searchParams.condition}; </span>}
        {searchParams.sponsor && <span>Sponsor = {searchParams.sponsor}; </span>}
        {searchParams.location?.latitude && (
          <span>
            Geo = distance({searchParams.location.latitude},{searchParams.location.longitude},
